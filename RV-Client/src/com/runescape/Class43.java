...
public class Class43
{
    ...
    public static void method371(CacheArchive class2, int i)
    {
        ByteBuffer byteStream = new ByteBuffer(class2.getDataForName("varp.dat"));
        if(i != 36135)
            return;
        anInt705 = 0;
        anInt703 = byteStream.getUnsignedShort();
    ...
    public void method372(int i, int j, ByteBuffer byteStream)
    {
        if(i >= 0)
            return;
        do
        {
            ...
            if(k == 0)
                return;
            if(k == 1)
                anInt708 = byteStream.getUnsignedByte();
            else
            if(k == 2)
                anInt709 = byteStream.getUnsignedByte();
            else
            if(k == 3)
            {
                aBoolean710 = true;
            ...
            if(k == 5)
                anInt712 = byteStream.getUnsignedShort();
            else
            if(k == 6)
                aBoolean713 = true;
            else
            if(k == 7)
                anInt714 = byteStream.getInt();
            else
            if(k == 8)
            {
                anInt715 = 1;
            ...
            if(k == 10)
                aString707 = byteStream.getRS2String();
            else
            if(k == 11)
                aBoolean716 = true;
            else
            if(k == 12)
                anInt717 = byteStream.getInt();
            else
            if(k == 13)
            {
                anInt715 = 2;
