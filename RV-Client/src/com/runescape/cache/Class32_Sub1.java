package com.runescape.cache;

import java.io.*;
import java.net.Socket;
import java.util.zip.CRC32;
import java.util.zip.GZIPInputStream;

import com.runescape.Class9;
import com.runescape.Client;
import com.runescape.NodeList;
import com.runescape.io.ByteBuffer;
import com.runescape.sign.Signlink;

public class Class32_Sub1 extends Class32 implements Runnable {
	
	private static final String VERSION_FILE_NAMES[] = { 
		"model_version", "anim_version", "midi_version", "map_version" 
	};

	public int anInt1334;
	public byte aByteArray1335[];
	public int anIntArray1336[];
	public byte aByteArrayArray1337[][];
	public boolean reading;
	public boolean aBoolean1339;
	public NodeList aClass6_1340;
	public int anInt1341;
	public int anInt1342;
	public int anInt1343;
	public int anIntArrayArray1344[][];
	public int anInt1345;
	public int anIntArray1346[];
	public String aString1347;
	public int anInt1348;
	public OutputStream outputStream;
	public int anInt1350;
	public NodeList aClass6_1351;
	public boolean aBoolean1352;
	public int anInt1353;
	public CRC32 aCRC32_1354;
	public Socket aSocket1355;
	public boolean aBoolean1356;
	public NodeList aClass6_1357;
	public NodeList aClass6_1358;
	public byte aByteArray1359[];
	public int anIntArray1360[];
	public int anInt1361;
	public int anInt1362;
	public int anInt1363;
	public byte readBuffer[];
	public int anIntArray1365[];
	public int anIntArray1366[];
	public int anInt1367;
	public int anInt1368;
	public Class9 aClass9_1369;
	public InputStream inputStream;
	public byte aByte1371;
	public Class50_Sub1_Sub3 aClass50_Sub1_Sub3_1372;
	public Client client;
	public NodeList aClass6_1374;
	public int anInt1375;
	public int anIntArray1376[];
	public long aLong1378;
	public int anInt1379;
	public boolean aBoolean1380;
	
	/**
	 * Actual version number of every single model/map/anim/midi in the cache?...
	 */
	public int versionNumbers[][];

	public void readInput() {
		try {
			int availableData = inputStream.available();
			if(anInt1362 == 0 && availableData >= 6) {
				reading = true;
				for(int j = 0; j < 6; j += inputStream.read(readBuffer, j, 6 - j));
				
				int k = readBuffer[0] & 0xff;
				int i1 = ((readBuffer[1] & 0xff) << 8) + (readBuffer[2] & 0xff);
				int k1 = ((readBuffer[3] & 0xff) << 8) + (readBuffer[4] & 0xff);
				int l1 = readBuffer[5] & 0xff;
				aClass50_Sub1_Sub3_1372 = null;
				for(Class50_Sub1_Sub3 class50_sub1_sub3 = (Class50_Sub1_Sub3) aClass6_1374.method158(); 
						class50_sub1_sub3 != null; 
						class50_sub1_sub3 = (Class50_Sub1_Sub3) aClass6_1374.method160(1)) {
					if(class50_sub1_sub3.anInt1467 == k && class50_sub1_sub3.anInt1468 == i1)
						aClass50_Sub1_Sub3_1372 = class50_sub1_sub3;
					if(aClass50_Sub1_Sub3_1372 != null)
						class50_sub1_sub3.anInt1469 = 0;
				}

				if(aClass50_Sub1_Sub3_1372 != null) {
					anInt1353 = 0;
					if(k1 == 0) {
						Signlink.reporterror("Rej: " + k + "," + i1);
						aClass50_Sub1_Sub3_1372.aByteArray1470 = null;
						if(aClass50_Sub1_Sub3_1372.aBoolean1471)
							synchronized(aClass6_1357) {
								aClass6_1357.method155(aClass50_Sub1_Sub3_1372);
							}
						else
							aClass50_Sub1_Sub3_1372.method442();
						aClass50_Sub1_Sub3_1372 = null;
					} else {
						if(aClass50_Sub1_Sub3_1372.aByteArray1470 == null && l1 == 0)
							aClass50_Sub1_Sub3_1372.aByteArray1470 = new byte[k1];
						if(aClass50_Sub1_Sub3_1372.aByteArray1470 == null && l1 != 0)
							throw new IOException("missing start of file");
					}
				}
				anInt1361 = l1 * 500;
				anInt1362 = 500;
				if(anInt1362 > k1 - l1 * 500)
					anInt1362 = k1 - l1 * 500;
			}
			if(anInt1362 > 0 && availableData >= anInt1362) {
				reading = true;
				byte abyte0[] = readBuffer;
				int l = 0;
				if(aClass50_Sub1_Sub3_1372 != null) {
					abyte0 = aClass50_Sub1_Sub3_1372.aByteArray1470;
					l = anInt1361;
				}
				for(int j1 = 0; j1 < anInt1362; j1 += inputStream.read(abyte0, j1 + l, anInt1362 - j1))
					;
				if(anInt1362 + anInt1361 >= abyte0.length && aClass50_Sub1_Sub3_1372 != null) {
					if(client.mainCacheFiles[0] != null)
						client.mainCacheFiles[aClass50_Sub1_Sub3_1372.anInt1467 + 1].method293(abyte0.length, true,
								abyte0, aClass50_Sub1_Sub3_1372.anInt1468);
					if(!aClass50_Sub1_Sub3_1372.aBoolean1471 && aClass50_Sub1_Sub3_1372.anInt1467 == 3) {
						aClass50_Sub1_Sub3_1372.aBoolean1471 = true;
						aClass50_Sub1_Sub3_1372.anInt1467 = 93;
					}
					if(aClass50_Sub1_Sub3_1372.aBoolean1471)
						synchronized(aClass6_1357) {
							aClass6_1357.method155(aClass50_Sub1_Sub3_1372);
						}
					else
						aClass50_Sub1_Sub3_1372.method442();
				}
				anInt1362 = 0;
				return;
			}
		} catch(IOException ioexception) {
			try {
				aSocket1355.close();
			} catch(Exception _ex) {
			}
			aSocket1355 = null;
			inputStream = null;
			outputStream = null;
			anInt1362 = 0;
		}
	}

	public int method325(int i, int j) {
		while(j >= 0)
			return anInt1367;
		return aByteArray1335[i] & 0xff;
	}

	public void method323(int i) {
		method329(0, i);
	}

	public void method326(int i) {
		if(i != 0)
			return;
		while(anInt1342 == 0 && anInt1343 < 10) {
			if(anInt1341 == 0)
				break;
			Class50_Sub1_Sub3 class50_sub1_sub3;
			synchronized(aClass6_1358) {
				class50_sub1_sub3 = (Class50_Sub1_Sub3) aClass6_1358.method157();
			}
			while(class50_sub1_sub3 != null) {
				if(aByteArrayArray1337[class50_sub1_sub3.anInt1467][class50_sub1_sub3.anInt1468] != 0) {
					aByteArrayArray1337[class50_sub1_sub3.anInt1467][class50_sub1_sub3.anInt1468] = 0;
					aClass6_1374.method155(class50_sub1_sub3);
					method342(anInt1345, class50_sub1_sub3);
					reading = true;
					if(anInt1334 < anInt1350)
						anInt1334++;
					aString1347 = "Loading extra files - " + (anInt1334 * 100) / anInt1350 + "%";
					anInt1343++;
					if(anInt1343 == 10)
						return;
				}
				synchronized(aClass6_1358) {
					class50_sub1_sub3 = (Class50_Sub1_Sub3) aClass6_1358.method157();
				}
			}
			for(int j = 0; j < 4; j++) {
				byte abyte0[] = aByteArrayArray1337[j];
				int k = abyte0.length;
				for(int l = 0; l < k; l++)
					if(abyte0[l] == anInt1341) {
						abyte0[l] = 0;
						Class50_Sub1_Sub3 class50_sub1_sub3_1 = new Class50_Sub1_Sub3();
						class50_sub1_sub3_1.anInt1467 = j;
						class50_sub1_sub3_1.anInt1468 = l;
						class50_sub1_sub3_1.aBoolean1471 = false;
						aClass6_1374.method155(class50_sub1_sub3_1);
						method342(anInt1345, class50_sub1_sub3_1);
						reading = true;
						if(anInt1334 < anInt1350)
							anInt1334++;
						aString1347 = "Loading extra files - " + (anInt1334 * 100) / anInt1350 + "%";
						anInt1343++;
						if(anInt1343 == 10)
							return;
					}

			}

			anInt1341--;
		}
	}

	public void method327(int i, int j, byte byte0, int k) {
		while(i >= 0)
			return;
		if(client.mainCacheFiles[0] == null)
			return;
		if(versionNumbers[j][k] == 0)
			return;
		byte abyte0[] = client.mainCacheFiles[j + 1].decompressFile(k);
		if(method341(abyte0, 764, versionNumbers[j][k], anIntArrayArray1344[j][k]))
			return;
		aByteArrayArray1337[j][k] = byte0;
		if(byte0 > anInt1341)
			anInt1341 = byte0;
		anInt1350++;
	}

	public boolean method328(int i, boolean flag) {
		if(!flag)
			throw new NullPointerException();
		return anIntArray1366[i] == 1;
	}

	public void method329(int i, int j) {
		if(i < 0 || i > versionNumbers.length || j < 0 || j > versionNumbers[i].length)
			return;
		if(versionNumbers[i][j] == 0)
			return;
		synchronized(aClass9_1369) {
			for(Class50_Sub1_Sub3 class50_sub1_sub3 = (Class50_Sub1_Sub3) aClass9_1369
					.method187(); class50_sub1_sub3 != null; class50_sub1_sub3 = (Class50_Sub1_Sub3) aClass9_1369
							.method188(1))
				if(class50_sub1_sub3.anInt1467 == i && class50_sub1_sub3.anInt1468 == j)
					return;

			Class50_Sub1_Sub3 class50_sub1_sub3_1 = new Class50_Sub1_Sub3();
			class50_sub1_sub3_1.anInt1467 = i;
			class50_sub1_sub3_1.anInt1468 = j;
			class50_sub1_sub3_1.aBoolean1471 = true;
			synchronized(aClass6_1340) {
				aClass6_1340.method155(class50_sub1_sub3_1);
			}
			aClass9_1369.method185(class50_sub1_sub3_1);
		}
	}

	public Class50_Sub1_Sub3 method330() {
		Class50_Sub1_Sub3 class50_sub1_sub3;
		synchronized(aClass6_1357) {
			class50_sub1_sub3 = (Class50_Sub1_Sub3) aClass6_1357.method157();
		}
		if(class50_sub1_sub3 == null)
			return null;
		synchronized(aClass9_1369) {
			class50_sub1_sub3.method443();
		}
		if(class50_sub1_sub3.aByteArray1470 == null)
			return class50_sub1_sub3;
		int i = 0;
		try {
			GZIPInputStream gzipinputstream = new GZIPInputStream(
					new ByteArrayInputStream(class50_sub1_sub3.aByteArray1470));
			do {
				if(i == aByteArray1359.length)
					throw new RuntimeException("buffer overflow!");
				int k = gzipinputstream.read(aByteArray1359, i, aByteArray1359.length - i);
				if(k == -1)
					break;
				i += k;
			} while(true);
		} catch(IOException _ex) {
			throw new RuntimeException("error unzipping");
		}
		class50_sub1_sub3.aByteArray1470 = new byte[i];
		for(int j = 0; j < i; j++)
			class50_sub1_sub3.aByteArray1470[j] = aByteArray1359[j];

		return class50_sub1_sub3;
	}

	public void run() {
		try {
			while(aBoolean1339) {
				anInt1348++;
				int i = 20;
				if(anInt1341 == 0 && client.mainCacheFiles[0] != null)
					i = 50;
				try {
					Thread.sleep(i);
				} catch(Exception _ex) {
				}
				reading = true;
				for(int j = 0; j < 100; j++) {
					if(!reading)
						break;
					reading = false;
					method338(true);
					method331(0);
					if(anInt1342 == 0 && j >= 5)
						break;
					method326(0);
					
					if(inputStream != null)
						readInput();
				}

				boolean flag = false;
				for(Class50_Sub1_Sub3 class50_sub1_sub3 = (Class50_Sub1_Sub3) aClass6_1374
						.method158(); class50_sub1_sub3 != null; class50_sub1_sub3 = (Class50_Sub1_Sub3) aClass6_1374
								.method160(1))
					if(class50_sub1_sub3.aBoolean1471) {
						flag = true;
						class50_sub1_sub3.anInt1469++;
						if(class50_sub1_sub3.anInt1469 > 50) {
							class50_sub1_sub3.anInt1469 = 0;
							method342(anInt1345, class50_sub1_sub3);
						}
					}

				if(!flag) {
					for(Class50_Sub1_Sub3 class50_sub1_sub3_1 = (Class50_Sub1_Sub3) aClass6_1374
							.method158(); class50_sub1_sub3_1 != null; class50_sub1_sub3_1 = (Class50_Sub1_Sub3) aClass6_1374
									.method160(1)) {
						flag = true;
						class50_sub1_sub3_1.anInt1469++;
						if(class50_sub1_sub3_1.anInt1469 > 50) {
							class50_sub1_sub3_1.anInt1469 = 0;
							method342(anInt1345, class50_sub1_sub3_1);
						}
					}

				}
				if(flag) {
					anInt1353++;
					if(anInt1353 > 750) {
						try {
							aSocket1355.close();
						} catch(Exception _ex) {
						}
						aSocket1355 = null;
						inputStream = null;
						outputStream = null;
						anInt1362 = 0;
					}
				} else {
					anInt1353 = 0;
					aString1347 = "";
				}
				if(client.loggedIn && aSocket1355 != null && outputStream != null
						&& (anInt1341 > 0 || client.mainCacheFiles[0] == null)) {
					anInt1375++;
					if(anInt1375 > 500) {
						anInt1375 = 0;
						readBuffer[0] = 0;
						readBuffer[1] = 0;
						readBuffer[2] = 0;
						readBuffer[3] = 10;
						try {
							outputStream.write(readBuffer, 0, 4);
						} catch(IOException _ex) {
							anInt1353 = 5000;
						}
					}
				}
			}
			return;
		} catch(Exception exception) {
			Signlink.reporterror("od_ex " + exception.getMessage());
		}
	}

	public void method331(int i) {
		anInt1342 = 0;
		anInt1343 = 0;
		if(i != 0)
			return;
		for(Class50_Sub1_Sub3 class50_sub1_sub3 = (Class50_Sub1_Sub3) aClass6_1374
				.method158(); class50_sub1_sub3 != null; class50_sub1_sub3 = (Class50_Sub1_Sub3) aClass6_1374
						.method160(1))
			if(class50_sub1_sub3.aBoolean1471)
				anInt1342++;
			else
				anInt1343++;

		while(anInt1342 < 10) {
			Class50_Sub1_Sub3 class50_sub1_sub3_1 = (Class50_Sub1_Sub3) aClass6_1351.method157();
			if(class50_sub1_sub3_1 == null)
				break;
			if(aByteArrayArray1337[class50_sub1_sub3_1.anInt1467][class50_sub1_sub3_1.anInt1468] != 0)
				anInt1334++;
			aByteArrayArray1337[class50_sub1_sub3_1.anInt1467][class50_sub1_sub3_1.anInt1468] = 0;
			aClass6_1374.method155(class50_sub1_sub3_1);
			anInt1342++;
			method342(anInt1345, class50_sub1_sub3_1);
			reading = true;
		}
	}

	public void method332(boolean flag, byte byte0) {
		if(byte0 != 109)
			aBoolean1352 = !aBoolean1352;
		int i = anIntArray1346.length;
		for(int j = 0; j < i; j++)
			if(flag || anIntArray1336[j] != 0) {
				method327(-44, 3, (byte) 2, anIntArray1365[j]);
				method327(-44, 3, (byte) 2, anIntArray1360[j]);
			}

	}

	public int method333() {
		synchronized(aClass9_1369) {
			int i = aClass9_1369.method189();
			return i;
		}
	}

	public boolean method334(int i, boolean flag) {
		for(int j = 0; j < anIntArray1346.length; j++)
			if(anIntArray1365[j] == i)
				return true;

		if(flag)
			anInt1363 = -405;
		return false;
	}

	public void method335(CacheArchive cacheArchive, Client client) {
		for(int versionFileIndex = 0; versionFileIndex < VERSION_FILE_NAMES.length; versionFileIndex++) {
			byte archiveData[] = cacheArchive.getDataForName(VERSION_FILE_NAMES[versionFileIndex]);
			ByteBuffer archiveBuffer = new ByteBuffer(archiveData);
			
			int itemCount = archiveData.length / 2;
			versionNumbers[versionFileIndex] = new int[itemCount];
			aByteArrayArray1337[versionFileIndex] = new byte[itemCount];
			
			for(int l = 0; l < itemCount; l++) {
				versionNumbers[versionFileIndex][l] = archiveBuffer.getUnsignedShort();
			}
		}

		String crcFileNames[] = { "model_crc", "anim_crc", "midi_crc", "map_crc" };
		for(int crcFileIndex = 0; crcFileIndex < 4; crcFileIndex++) {
			byte archiveData[] = cacheArchive.getDataForName(crcFileNames[crcFileIndex]);
			int i1 = archiveData.length / 4;
			ByteBuffer buffer = new ByteBuffer(archiveData);
			anIntArrayArray1344[crcFileIndex] = new int[i1];
			for(int l1 = 0; l1 < i1; l1++)
				anIntArrayArray1344[crcFileIndex][l1] = buffer.getInt();

		}

		byte abyte2[] = cacheArchive.getDataForName("model_index");
		int j1 = versionNumbers[0].length;
		aByteArray1335 = new byte[j1];
		for(int k1 = 0; k1 < j1; k1++)
			if(k1 < abyte2.length)
				aByteArray1335[k1] = abyte2[k1];
			else
				aByteArray1335[k1] = 0;

		abyte2 = cacheArchive.getDataForName("map_index");
		ByteBuffer byteStream_2 = new ByteBuffer(abyte2);
		j1 = abyte2.length / 7;
		anIntArray1346 = new int[j1];
		anIntArray1360 = new int[j1];
		anIntArray1365 = new int[j1];
		anIntArray1336 = new int[j1];
		for(int i2 = 0; i2 < j1; i2++) {
			anIntArray1346[i2] = byteStream_2.getUnsignedShort();
			anIntArray1360[i2] = byteStream_2.getUnsignedShort();
			anIntArray1365[i2] = byteStream_2.getUnsignedShort();
			anIntArray1336[i2] = byteStream_2.getUnsignedByte();
		}

		abyte2 = cacheArchive.getDataForName("anim_index");
		byteStream_2 = new ByteBuffer(abyte2);
		j1 = abyte2.length / 2;
		anIntArray1376 = new int[j1];
		for(int j2 = 0; j2 < j1; j2++)
			anIntArray1376[j2] = byteStream_2.getUnsignedShort();

		abyte2 = cacheArchive.getDataForName("midi_index");
		byteStream_2 = new ByteBuffer(abyte2);
		j1 = abyte2.length;
		anIntArray1366 = new int[j1];
		for(int k2 = 0; k2 < j1; k2++)
			anIntArray1366[k2] = byteStream_2.getUnsignedByte();

		this.client = client;
		aBoolean1339 = true;
		client.method12(this, 2);
	}

	public void method336(byte byte0) {
		synchronized(aClass6_1358) {
			aClass6_1358.method162();
		}
		if(byte0 != -125)
			aBoolean1352 = !aBoolean1352;
	}

	public void method337(int i, int j, byte byte0) {
		if(client.mainCacheFiles[0] == null)
			return;
		if(versionNumbers[j][i] == 0)
			return;
		if(aByteArrayArray1337[j][i] == 0)
			return;
		if(anInt1341 == 0)
			return;
		Class50_Sub1_Sub3 class50_sub1_sub3 = new Class50_Sub1_Sub3();
		if(byte0 != -113)
			anInt1367 = 244;
		class50_sub1_sub3.anInt1467 = j;
		class50_sub1_sub3.anInt1468 = i;
		class50_sub1_sub3.aBoolean1471 = false;
		synchronized(aClass6_1358) {
			aClass6_1358.method155(class50_sub1_sub3);
		}
	}

	public void method338(boolean flag) {
		Class50_Sub1_Sub3 class50_sub1_sub3;
		synchronized(aClass6_1340) {
			class50_sub1_sub3 = (Class50_Sub1_Sub3) aClass6_1340.method157();
		}
		if(!flag) {
			for(int i = 1; i > 0; i++)
				;
		}
		while(class50_sub1_sub3 != null) {
			reading = true;
			byte abyte0[] = null;
			if(client.mainCacheFiles[0] != null)
				abyte0 = client.mainCacheFiles[class50_sub1_sub3.anInt1467 + 1].decompressFile(class50_sub1_sub3.anInt1468);
			if(!method341(abyte0, 764, versionNumbers[class50_sub1_sub3.anInt1467][class50_sub1_sub3.anInt1468],
					anIntArrayArray1344[class50_sub1_sub3.anInt1467][class50_sub1_sub3.anInt1468]))
				abyte0 = null;
			synchronized(aClass6_1340) {
				if(abyte0 == null) {
					aClass6_1351.method155(class50_sub1_sub3);
				} else {
					class50_sub1_sub3.aByteArray1470 = abyte0;
					synchronized(aClass6_1357) {
						aClass6_1357.method155(class50_sub1_sub3);
					}
				}
				class50_sub1_sub3 = (Class50_Sub1_Sub3) aClass6_1340.method157();
			}
		}
	}

	public void method339() {
		aBoolean1339 = false;
	}

	public int method340(int i, int j) {
		if(j != -31140)
			aBoolean1380 = !aBoolean1380;
		return versionNumbers[i].length;
	}

	public boolean method341(byte abyte0[], int i, int j, int k) {
		i = 22 / i;
		if(abyte0 == null || abyte0.length < 2)
			return false;
		int l = abyte0.length - 2;
		int i1 = ((abyte0[l] & 0xff) << 8) + (abyte0[l + 1] & 0xff);
		aCRC32_1354.reset();
		aCRC32_1354.update(abyte0, 0, l);
		int j1 = (int) aCRC32_1354.getValue();
		if(i1 != j)
			return false;
		return j1 == k;
	}

	public void method342(int i, Class50_Sub1_Sub3 class50_sub1_sub3) {
		if(i != 0)
			return;
		try {
			if(aSocket1355 == null) {
				long l = System.currentTimeMillis();
				if(l - aLong1378 < 4000L)
					return;
				aLong1378 = l;
				aSocket1355 = client.openSocket(43594 + Client.portOffset);
				inputStream = aSocket1355.getInputStream();
				outputStream = aSocket1355.getOutputStream();
				outputStream.write(15);
				for(int j = 0; j < 8; j++)
					inputStream.read();

				anInt1353 = 0;
			}
			readBuffer[0] = (byte) class50_sub1_sub3.anInt1467;
			readBuffer[1] = (byte) (class50_sub1_sub3.anInt1468 >> 8);
			readBuffer[2] = (byte) class50_sub1_sub3.anInt1468;
			if(class50_sub1_sub3.aBoolean1471)
				readBuffer[3] = 2;
			else if(!client.loggedIn)
				readBuffer[3] = 1;
			else
				readBuffer[3] = 0;
			outputStream.write(readBuffer, 0, 4);
			anInt1375 = 0;
			anInt1379 = -10000;
			return;
		} catch(IOException ioexception) {
		}
		try {
			aSocket1355.close();
		} catch(Exception _ex) {
		}
		aSocket1355 = null;
		inputStream = null;
		outputStream = null;
		anInt1362 = 0;
		anInt1379++;
	}

	public int method343(int i) {
		i = 0 / i;
		return anIntArray1376.length;
	}

	public int method344(int i, int j, int k, int l) {
		if(i != 0)
			return 1;
		int i1 = (j << 8) + k;
		for(int j1 = 0; j1 < anIntArray1346.length; j1++) {
			if(anIntArray1346[j1] == i1) {
				if(l == 0) {
					System.out.println("MAP " + anIntArray1360[j1]);
					return anIntArray1360[j1];
				} else {
					return anIntArray1365[j1];
				}
			}
		}

		return -1;
	}

	public Class32_Sub1() {
		aByteArrayArray1337 = new byte[4][];
		reading = false;
		aBoolean1339 = true;
		aClass6_1340 = new NodeList(true);
		anIntArrayArray1344 = new int[4][];
		aString1347 = "";
		aClass6_1351 = new NodeList(true);
		aBoolean1352 = false;
		aCRC32_1354 = new CRC32();
		aBoolean1356 = false;
		aClass6_1357 = new NodeList(true);
		aClass6_1358 = new NodeList(true);
		aByteArray1359 = new byte[65000];
		readBuffer = new byte[500];
		anInt1367 = 591;
		aClass9_1369 = new Class9(true);
		aByte1371 = 6;
		aClass6_1374 = new NodeList(true);
		versionNumbers = new int[4][];
		aBoolean1380 = false;
	}

}
