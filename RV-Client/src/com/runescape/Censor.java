...
public class Censor {
	...
	public static void unpackFilters(CacheArchive archive) {
		ByteBuffer fragmentsencBuffer = new ByteBuffer(archive.getDataForName("fragmentsenc.txt"));
		ByteBuffer badencBuffer = new ByteBuffer(archive.getDataForName("badenc.txt"));
		ByteBuffer domainencBuffer = new ByteBuffer(archive.getDataForName("domainenc.txt"));
		ByteBuffer tldlistBuffer = new ByteBuffer(archive.getDataForName("tldlist.txt"));
		method374(fragmentsencBuffer, badencBuffer, domainencBuffer, tldlistBuffer);
	}

	public static void method374(ByteBuffer fragmentsencBuffer, ByteBuffer badencBuffer, ByteBuffer domainencBuffer, ByteBuffer tldlistBuffer) {
		unpackBadenc(badencBuffer);
		unpackDomainenc(domainencBuffer);
		unpackFragmentsenc(fragmentsencBuffer);
		unackTldlist(tldlistBuffer);
	}

	public static void unackTldlist(ByteBuffer tldlistBuffer) {
		int j = tldlistBuffer.getInt();
		aCharArrayArray744 = new char[j][];
		anIntArray745 = new int[j];
		for(int k = 0; k < j; k++) {
			anIntArray745[k] = tldlistBuffer.getUnsignedByte();
			char ac[] = new char[tldlistBuffer.getUnsignedByte()];
			for(int l = 0; l < ac.length; l++)
				ac[l] = (char) tldlistBuffer.getUnsignedByte();

			aCharArrayArray744[k] = ac;
		}
	}

	public static void unpackBadenc(ByteBuffer badencBuffer) {
		int i = badencBuffer.getInt();
		aCharArrayArray741 = new char[i][];
		aByteArrayArrayArray742 = new byte[i][][];
		method379(badencBuffer, aCharArrayArray741, aByteArrayArrayArray742);
	}

	public static void unpackDomainenc(ByteBuffer domainencBuffer) {
		int j = domainencBuffer.getInt();
		aCharArrayArray743 = new char[j][];
		method380(domainencBuffer, aCharArrayArray743);
	}

	public static void unpackFragmentsenc(ByteBuffer fragmentsencBuffer) {
		anIntArray740 = new int[fragmentsencBuffer.getInt()];
		for(int j = 0; j < anIntArray740.length; j++)
			anIntArray740[j] = fragmentsencBuffer.getUnsignedShort();

	}

	public static void method379(ByteBuffer byteStream, char ac[][], byte abyte0[][][]) {
		for(int j = 0; j < ac.length; j++) {
			char ac1[] = new char[byteStream.getUnsignedByte()];
			for(int k = 0; k < ac1.length; k++)
				ac1[k] = (char) byteStream.getUnsignedByte();

			ac[j] = ac1;
			byte abyte1[][] = new byte[byteStream.getUnsignedByte()][2];
			for(int l = 0; l < abyte1.length; l++) {
				abyte1[l][0] = (byte) byteStream.getUnsignedByte();
				abyte1[l][1] = (byte) byteStream.getUnsignedByte();
			}
	...
	public static void method380(ByteBuffer byteStream, char ac[][]) {
		for(int j = 0; j < ac.length; j++) {
			char ac1[] = new char[byteStream.getUnsignedByte()];
			for(int k = 0; k < ac1.length; k++)
				ac1[k] = (char) byteStream.getUnsignedByte();
	...
	public static final String aStringArray746[] = { "cook", "cook's", "cooks", "seeks", "sheet", "woop", "woops", "faq", "noob", "noobs" };
