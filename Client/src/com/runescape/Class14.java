...
public class Class14
{
    ...
    public static void method204(CacheArchive class2, int i)
    {
        ByteBuffer byteStream = new ByteBuffer(class2.getDataForName("seq.dat"));
        anInt292 = byteStream.getUnsignedShort();
    ...
    public void method206(byte byte0, ByteBuffer byteStream)
    {
        if(byte0 == 6)
            byte0 = 0;
        else
        ...
        do
        {
            ...
            if(i == 1)
            {
                anInt294 = byteStream.getUnsignedByte();
                anIntArray295 = new int[anInt294];
                anIntArray296 = new int[anInt294];
                anIntArray297 = new int[anInt294];
                for(int j = 0; j < anInt294; j++)
                {
                    anIntArray295[j] = byteStream.getUnsignedShort();
                    anIntArray296[j] = byteStream.getUnsignedShort();
                    if(anIntArray296[j] == 65535)
                        anIntArray296[j] = -1;
                    anIntArray297[j] = byteStream.getUnsignedShort();
                }

            } else
            if(i == 2)
                anInt298 = byteStream.getUnsignedShort();
            else
            if(i == 3)
            {
                ...
                for(int l = 0; l < k; l++)
                    anIntArray299[l] = byteStream.getUnsignedByte();
            ...
            if(i == 5)
                anInt301 = byteStream.getUnsignedByte();
            else
            if(i == 6)
                anInt302 = byteStream.getUnsignedShort();
            else
            if(i == 7)
                anInt303 = byteStream.getUnsignedShort();
            else
            if(i == 8)
                anInt304 = byteStream.getUnsignedByte();
            else
            if(i == 9)
                anInt305 = byteStream.getUnsignedByte();
            else
            if(i == 10)
                anInt306 = byteStream.getUnsignedByte();
            else
            if(i == 11)
                anInt307 = byteStream.getUnsignedByte();
            else
            if(i == 12)
                anInt308 = byteStream.getInt();
            else
