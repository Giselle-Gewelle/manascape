...
public class Class38
{
    ...
    public static void method365(ByteBuffer byteStream, int i)
    {
        if(i != 36135)
            return;
        aByteArray670 = new byte[0x6baa8];
        aByteStream_671 = new ByteBuffer(aByteArray670);
        Class11.method190();
        do
        {
            int j = byteStream.getUnsignedShort();
            if(j == 65535)
                return;
    ...
    public static ByteBuffer method366(int i, byte byte0, int j)
    {
        if(byte0 != 6)
    ...
    public void method367(byte byte0, ByteBuffer byteStream)
    {
        for(int i = 0; i < 10; i++)
        {
        ...
        anInt673 = byteStream.getUnsignedShort();
        anInt674 = byteStream.getUnsignedShort();
        if(byte0 == 6)
        {
            byte0 = 0;
            return;
    ...
    public ByteBuffer method369(int i, int j)
    {
        int k = method370(j);
        aByteStream_671.position = 0;
        aByteStream_671.putInt(0x52494646);
        aByteStream_671.putLEInt(36 + k, false);
        aByteStream_671.putInt(0x57415645);
        aByteStream_671.putInt(0x666d7420);
        if(i >= 0)
        {
            throw new NullPointerException();
        } else
        {
            aByteStream_671.putLEInt(16, false);
            aByteStream_671.putLEShort(1);
            aByteStream_671.putLEShort(1);
            aByteStream_671.putLEInt(22050, false);
            aByteStream_671.putLEInt(22050, false);
            aByteStream_671.putLEShort(1);
            aByteStream_671.putLEShort(8);
            aByteStream_671.putInt(0x64617461);
            aByteStream_671.putLEInt(k, false);
            aByteStream_671.position += k;
    ...
    public static ByteBuffer aByteStream_671;
