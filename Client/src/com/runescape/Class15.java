...
public class Class15 {
	...
	public static void method207(CacheArchive class2, int i) {
		if(i != 36135)
			return;
		ByteBuffer byteStream = new ByteBuffer(class2.getDataForName("flo.dat"));
		anInt313 = byteStream.getUnsignedShort();
	...
	public void method208(byte byte0, ByteBuffer byteStream) {
		if(byte0 == 6) {
			byte0 = 0;
		} else {
			for(int i = 1; i > 0; i++)
		...
		do {
			...
			if(j == 0)
				return;
			if(j == 1) {
				anInt316 = byteStream.getTriByte();
				method209(true, anInt316);
			} else if(j == 2)
				anInt317 = byteStream.getUnsignedByte();
			else if(j == 3)
				aBoolean318 = true;
			else if(j == 5)
				aBoolean319 = false;
			else if(j == 6)
				aString315 = byteStream.getRS2String();
			else if(j == 7) {
				int k = anInt320;
				int l = anInt321;
				int i1 = anInt322;
				int j1 = anInt323;
				int k1 = byteStream.getTriByte();
				method209(true, k1);
				anInt320 = k;
				anInt321 = l;
				anInt322 = i1;
