package com.runescape.gameserver.world.container;
...
public class Bank {
	...
	public static void open(Player player) {
		...
		player.getPacketSender().sendInterfaceInventory(5292, 5063);
	...
	public static void withdraw(Player player, int slot, int id, int amount) {
		...
		if(player.getSettings().getBool("withdrawNotes")) {
		...
		if(def.isStackable()) {
			if(player.getInventory().freeSlots() <= 0 && player.getInventory().getById(newId) == null) {
				player.getPacketSender().sendMessage("You don't have enough inventory space to withdraw that many."); // this is the real message
			}
		} else {
		...
		try {
			...
			if(item.getDefinition().isStackable() || noted) {
				...
				if(player.getBank().freeSlots() < 1 && player.getBank().getById(bankedId) == null) {
					player.getPacketSender().sendMessage("You don't have enough space in your bank account."); // this is the real message
				}
