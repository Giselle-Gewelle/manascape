...
public final class ImageServlet extends HttpServlet {
	...
	private static final long serialVersionUID = -6352788526876350341L;
	
	private static final Logger LOG = Logger.getLogger(ImageServlet.class);
	
	private static final String 
		NEWS_DIR = System.getenv("TOMCAT_HOME") + File.separatorChar + "uploads" + File.separatorChar + "manascape" + File.separatorChar + "newsIcons",
		USER_DIR = System.getenv("TOMCAT_HOME") + File.separatorChar + "uploads" + File.separatorChar + "manascape" + File.separatorChar + "userUploads";
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response) {
		...
		File file = new File(System.getenv("TOMCAT_HOME") + File.separatorChar + "uploads" + File.separatorChar + "manascape" + File.separatorChar + imageName + ".png");
		if(!file.exists()) {
			try {
	...
	private void verifyDirectories() {
		...
		File newsIconFolder = new File(NEWS_DIR);
		File userUploadFolder = new File(USER_DIR);
		if(!newsIconFolder.exists()) {
			newsIconFolder.mkdirs();
